...
- Läuft als geplanter Job (eine Pipeline, Schleife über Mandanten) und als Worker-Container (`python -m stromkreis_pipeline worker`), der die von der Plattform beim Login eingestellten Import-Aufträge abarbeitet: erst Teilnehmer und Zählpunkte (anschließend Geokodierung geänderter Mitgliedsadressen per Nominatim in `member.latitude/longitude`; Anlagen, die noch die alte Mitgliedsadresse oder den Gemeinschafts-Mittelpunkt tragen, ziehen mit), dann Energiedaten in mengenabhängigen Stücken (Ziel ~500k Werte je Anfrage: kleine EEGs 30 Tage, ~700 Zählpunkte 7 Tage) mit Lastschutz-Pause (mindestens `WORKER_PACE_SECONDS`, bei langsamen Antworten die doppelte Antwortzeit); nach jedem Stück wird das Tagesaggregat `measurement_daily` für den Zeitraum neu gerechnet (DB-Funktion `refresh_measurement_daily`), auf dem der Tab "Energie" der Plattform arbeitet. Auth über die Bearer-Routen von EEG-Faktura (`/api/participant`, `/energystore/eeg/v2/{ecid}/meta|raw`) mit dem Refresh-Token des Betreibers
...
- `battery_site`: Anlage (openHABian-Gateway) mit Wechselrichterprofil, Gateway-Token (nur SHA-256-Hash), Standort (latitude/longitude für die Karte) und letztem Status-Push (`status` jsonb, `last_seen_at`); Ladefenster-Konfiguration und Status-Verlauf folgen mit der IBM-API
