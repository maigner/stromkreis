...
- **Neu am 8.8., lokal getestet, noch nicht am Server** (kommt mit dem nächsten Deploy): `energy_series_weather` mit `meter_code` (EEG-Faktura-Kategorien je Mandant, stabiler `kind`-Schlüssel: total_consumption, production_share, self_use, total_production, overshoot), `measurement` (15-Minuten-Reihen je Zählpunkt und Kategorie, kWh, UTC-Intervallbeginn, zusammengesetzter PK ohne Surrogat-id) und `weather` (Stundenwetter je Mandant, Spaltensatz wie ISCHLSTROM-Loader inkl. Pflichtfeld-Aufteilung). Zusammengesetzte FKs und Check-Constraints per Smoke-Test verifiziert (Cross-Tenant-Inserts scheitern). `schema.sql` per Container-`pg_dump` aktualisiert.
...
4. **Nächster Roadmap-Schritt (Phase 1):** Demo-Datengenerator für Salzkammerstrom, dann EEG-Faktura-Importer in `pipeline/` gegen echte ISCHLSTROM-Daten (Portierungsquellen siehe `architektur.md`). Das Schema für 15-Minuten-Reihen und Wetterdaten ist seit 8.8. vorhanden (`energy_series_weather`).
...
6. Git macht Martin weiterhin manuell; die Änderungen vom 8.8. (dieser Stand, `.gitignore`, `CLAUDE.md`, `architektur.md`, `roadmap.md`, Migration `energy_series_weather` samt `schema.sql`) sind noch nicht committet.
