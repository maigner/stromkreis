...
- `gateway/` — per-inverter-profile packages installed at members' homes (hardware list, setup guide, control logic). Gateways poll the platform outbound via HTTPS with per-site tokens; remote maintenance runs over an outbound-initiated WireGuard tunnel into the server's maintenance net (SSH console on the site detail page). Nothing is opened on members' home routers.
- `docs/` — vision, architecture, roadmap, current working status (`status.md`) (German).
- `deploy/` — test deployment: compose stack (Postgres, platform, pipeline worker, WireGuard, openHAB cloud, dbmate migrate) on the homelab host `server` at `/home/martin/Container/stromkreis`, deployed via `deploy/deploy.sh` (rsync source, build on server; no git involved). Platform listens on 127.0.0.1:4000 behind the server's Caddy, publicly reachable at https://stromkreis.net; secrets live only in the server-side `.env`.
